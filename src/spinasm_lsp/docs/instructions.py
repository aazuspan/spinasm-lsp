...
@dataclass
class InstructionDocumentation(MarkdownDocumentationGenerator):
    ...
    def __post_init__(self):
        if isinstance(self.args, InstructionArg):
            self.args = [self.args]
    ...
    def markdown(self) -> str:
        ...
        md = MarkdownString()
        signature = f"`{self.name} {', '.join([arg.name for arg in self.args])}`"

        md.add_heading(signature, level=2)
        md.add_horizontal_rule()

        md.add_paragraph(self.description.strip())

        md.add_heading("Operation", level=3)
        md.add_paragraph(f"`{self.operation}`")

        md.add_heading("Parameters", level=3)
        if not self.args:
        ...
        md.add_heading("Instruction Coding", level=3)
        md.add_paragraph(f"**{self.coding}**")
...
INSTRUCTIONS = {
    "ABSA": InstructionDocumentation(
        name="ABSA",
        args=[],
        description="""Loads the accumulator with the absolute value of the accumulator.""",
...
""",
    ...
    "AND": InstructionDocumentation(
        name="AND",
        args=[
            InstructionArg("M", 24, ["Binary", "Hex ($000000 - $FFFFFF)", "Symbolic"]),
        ],
        description="""AND will perform a bit wise "and" of the current ACC and the 24-­bit MASK specified within the instruction word. The instruction might be used to load a constant into ACC provided ACC contains $FFFFFF or to clear ACC if MASK equals $000000. (see also the pseudo opcode section)""",
        parameter_description=None,
        operation="ACC & MASK",
...
""",
    ...
    "CHO RDAL": InstructionDocumentation(
        name="CHO RDAL",
        args=[
            InstructionArg("N", 2, ["LFO select: SIN0,COS0,SIN1,COS1,RMP0,RMP1"]),
        ],
        description="""`CHO RDAL` will read the current value of the selected LFO into `ACC`.""",
        parameter_description=None,
        operation="LFO * 1 ­> ACC",
...
""",
    ...
    "CHO SOF": InstructionDocumentation(
        name="CHO SOF",
        args=[
            InstructionArg("N", 2, ["LFO select: SIN0,SIN1,RMP0,RMP1"]),
            InstructionArg("C", 6, ["Binary", "Bit flags"]),
...
""",
    ...
    "EXP": InstructionDocumentation(
        name="EXP",
        args=[
            InstructionArg(
                "C", 16, ["Real (S1.14)", "Hex ($0000 - $FFFF)", "Symbolic"]
            ),
            InstructionArg("D", 11, ["Real (S.10)", "Symbolic"]),
        ],
        description="""`EXP` will multiply 2^`ACC` with `C` and add the constant `D` to the result.
...
""",
    ...
    "JAM": InstructionDocumentation(
        name="JAM",
        args=[
            InstructionArg("N", 1, ["RAMP LFO select: (0, 1)"]),
        ],
        description="""`JAM` will reset the selected RAMP LFO to its starting point.""",
        parameter_description=None,
...
""",
    ...
    "LDAX": InstructionDocumentation(
        name="LDAX",
        args=[
            InstructionArg(
                "ADDR", 6, ["Decimal (0 - 63)", "Hex ($0 - $3F)", "Symbolic"]
            ),
        ],
        description="""Loads the accumulator with the contents of the addressed register.""",
        parameter_description=None,
...
""",
    ...
    "LOG": InstructionDocumentation(
        name="LOG",
        args=[
            InstructionArg(
                "C", 16, ["Real (S1.14)", "Hex ($0000 - $FFFF)", "Symbolic"]
            ),
            InstructionArg("D", 11, ["Real (S4.6)", "Symbolic"]),
        ],
        description="""`LOG` will multiply the Base2 `LOG` of the current absolute value in `ACC` with `C` and add the constant `D` to the result.
...
""",
    ...
    "MAXX": InstructionDocumentation(
        ...
        args=[
            ...
            InstructionArg(
                "C", 16, ["Real (S1.14)", "Hex ($8000 - $0000 - $7FFF)", "Symbolic"]
            ),
        ],
        description="""`MAXX` will compare the absolute value of `ACC` versus C times the absolute value of the register pointed to by `ADDR`. If the absolute value of `ACC` is larger `ACC` will be loaded with `|ACC|`, otherwise the accumulator becomes overwritten by `|REG[ADDR] * C|`.""",
...
""",
    ...
    "MULX": InstructionDocumentation(
        ...
        description="""`MULX` will multiply `ACC` by the value of the register pointed to by `ADDR`. An important application of the `MULX` instruction is squaring the content of `ACC`, which combined with a single order LP is especially useful in calculating the RMS value of an arbitrary waveform.""",
...
""",
    ...
    "OR": InstructionDocumentation(
        name="OR",
        args=[
            InstructionArg("M", 24, ["Binary", "Hex ($000000 - $FFFFFF)", "Symbolic"]),
        ],
        description="""`OR` will perform a bit wise "or" of the current `ACC` and the 24-­bit MASK specified within the instruction word. The instruction might be used to load a constant into `ACC` provided `ACC` contains `$000000`.""",
        parameter_description=None,
        operation="ACC | MASK",
...
""",
    ...
    "RDA": InstructionDocumentation(
        ...
        args=[
            ...
            InstructionArg(
                "C", 11, ["Real (S1.9)", "Hex ($400 - $000 - $3FF)", "Symbolic"]
            ),
        ],
        description="""RDA will fetch the sample [ADDR] from the delay RAM, multiply it by C, and add the result to the previous content of ACC. This multiply-accumulate operation is probably the most popular operation found in DSP algorithms.""",
        parameter_description=None,
...
""",
    ...
    "RDAX": InstructionDocumentation(
        ...
        description="""RDAX will fetch the value contained in `[ADDR]` from the register file, multiply it with `C` and add the result to the previous content of `ACC`. This multiply accumulate is probably the most popular operation found in DSP algorithms.""",
...
""",
    ...
    "RDFX": InstructionDocumentation(
        ...
        description="""`RDFX` will subtract the value of the register pointed to by `ADDR` from `ACC`, multiply the result by `C` and then add the value of the register pointed to by `ADDR`. `RDFX` is an extremely powerful instruction in that it represents the major portion of a single order low pass filter.""",
...
""",
    ...
    "RMPA": InstructionDocumentation(
        ...
        description="""`RMPA` provides indirect delay line addressing in that the delay line address of the sample to be multiplied by `C` is not explicitly given in the instruction itself but contained within the pointer register `ADDR_PTR` (absolute address 24 within the internal register file.)
...
""",
    ...
    "SOF": InstructionDocumentation(
        ...
        description="""SOF will multiply the current value in ACC with C and will then add the constant D to the result.
...
""",
    ...
    "WLDS": InstructionDocumentation(
        name="WLDS",
        args=[
            InstructionArg("N", 1, ["SIN LFO select: (0, 1)"]),
            InstructionArg(
                "F", 9, ["Decimal (0 - 511)", "Hex ($000 - $1FF)", "Symbolic"]
            ),
            InstructionArg(
                "A", 15, ["Decimal (0 - 32767)", "Hex ($0000 - $7FFF)", "Symbolic"]
            ),
        ],
        description="""`WLDS` will load frequency and amplitude control values into the selected SIN LFO (0 or 1). This instruction is intended to setup the selected SIN LFO which is typically done within the first sample iteration after a new program is loaded. As a result `WLDS` will in most cases be used in combination with a `SKP RUN` instruction. For a more detailed description regarding the frequency and amplitude control values see application note AN­0001.""",
        parameter_description=None,
        operation="See description",
...
""",
    ...
    "WRA": InstructionDocumentation(
        ...
        description="""`WRA` will store `ACC` to the delay ram location addressed by `ADDR` and then multiply `ACC` by `C`.""",
        parameter_description=None,
...
""",
    ...
    "WRAX": InstructionDocumentation(
        ...
        description="""`WRAX` will save the current value in `ACC` to `[ADDR]` and then multiply `ACC` by `C`. This instruction can be used to write `ACC` to one DAC channel while clearing `ACC` for processing the next audio channel.""",
...
""",
    ...
    "WRHX": InstructionDocumentation(
        ...
        description="""The current ACC value is stored in the register pointed to by ADDR, then ACC is multiplied by C. Finally, the previous content of ACC (PACC) is added to the product. WRHX is an extremely powerful instruction; when combined with RDFX, it forms a single order high pass shelving filter.""",
...
""",
    ...
    "WRLX": InstructionDocumentation(
        ...
        description="""First, the current ACC value is stored into the register pointed to by ADDR, then ACC is subtracted from the previous content of ACC (PACC). The difference is then multiplied by C and finally PACC is added to the result. WRLX is an extremely powerful instruction in that when combined with RDFX, it forms a single order low-pass shelving filter.""",
...
""",
    ...
    "XOR": InstructionDocumentation(
        name="XOR",
        args=[
            InstructionArg("M", 24, ["Binary", "Hex ($000000 - $FFFFFF)", "Symbolic"]),
        ],
        description="""`XOR` will perform a bit wise "xor" of the current `ACC` and the 24­-bit MASK specified within the instruction word. The instruction will invert `ACC` provided `MASK` equals `$FFFFFF`. (see also the pseudo opcode section).""",
        parameter_description=None,
        operation="ACC ^ MASK",
